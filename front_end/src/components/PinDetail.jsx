    ...
    <div className = 'flex xl-flex-row flex-col m-auto bg-white' style={{maxWidth:'1500px',borderRadius:"32px"}}>
      ...
      <div className = 'w-full p-5 flex-1 xl:min-w-629'>
          <div className = "flex items-center justify-between">
            ...
            <a href={pinDetail.destination} target ="_blank" >{pinDetail.destination}</a>
        ...
        <div className = "max-h-370 overflow-y-auto">
            {
              pinDetail?.comments?.map((comment,i)=> (
                <div className = "flex gap-2 mt-5 items-center bg-white rounded-lg" key ={i}>
                    <img src={comment.postedBy.image} className = 'w-10 h-10 rounded-full cursor-pointer'/>
        ...
        <Link to = {`user-profile/${pinDetail?.postedBy._id}`} >
            <img src = {pinDetail?.postedBy.image} className = "w-10 h-10 rounded-full mr-5 cursor-pointer " alt ="user-profile"/>
        </Link >

        <input className ='flex-1 border-gray-100 outline-none border-2 p-2 rounded-2xl focus:border-gray-300' type = "text" placeholder = "Add Comment" value = {comment} onChange = {(e)=>setComment(e.target.value)}  />
